<template>
  <section>
    <d2-container type="full" v-if="!show.edit&&!show.view">
      <section class="data-list-box">
        <mk-search-form :model="query" @search="goPage(1)">
          <el-form-item class="inline-item" prop="pro_title">
            <el-input v-model.trim="query.params.pro_title" @clear="goPage(1)" @keyup.native.enter="goPage(1)"
                      clearable placeholder="输入项目组名称搜索"
                      size="mini"></el-input>
          </el-form-item>
          ...
          <el-form-item class="inline-item" prop="remark">
            <el-input v-model.trim="query.params.remark" @clear="goPage(1)" @keyup.native.enter="goPage(1)"
                      clearable placeholder="输入项目组副标题搜索"
                      size="mini"></el-input>
          </el-form-item>
        </mk-search-form>
    ...
    <dir-analysis-info v-if="show.view" :updateData="updateData" @close="close"/>
  </section>
</template>
...
import {dirProanalysisDelete, dirProanalysisList} from "@/api/modules/sys.proanalysis.api";
import GetProanalysisInfo from "./get-proanalysis-info";
import ViewProanalysisInfo from "./view-proanalysis-info";
import DirAnalysisInfo from "@/views/system/analysis/dir-analysis-info";

export default {
  name: 'dir-proanalysis-info',
  components:{DirAnalysisInfo, ViewProanalysisInfo, GetProanalysisInfo,},
  data() {
    return {
      show: {
        edit: false,
      ...
      fields: [
        ...
        {key: 'pro_title', name: '项目组标题', show: true, align: "center", enableSort: false, fixed: false},
        {key: 'remark', name: '项目组副标题', show: true, align: "center", enableSort: false, fixed: false},
  ...
  methods: {
    ...
    getDataList() {
      try {
        ...
        dirProanalysisList(this.query).then(res => {
          let datas = res.data || []
    ...
    goDelete(data) {
      this.$toast.confirm('此操作将永久删除该条数据, 是否继续?').then(() => {
        dirProanalysisDelete(data.id).then(res => {
    ...
    //添加编辑
    goEdit(data) {
      this.$toast.showSmallModal(data && data.id ? '修改' : '增加', GetProanalysisInfo, {updateData: data}, data => {
        if (data) {
          this.getDataList()
        }
      })
