...
<template>
	<view class="components-theme">
		...
		<view class='UCenter-bg' :style="'background-image: url(' + pic[topBackGroupImageIndex].link + ');margin-top:' + CustomBar + 'px;'">
            ...
            <image src='../../static/logo.png' class='me-logo' mode='aspectFit'></image>
					</view>
		...
		<block>
			<view class='padding flex text-center text-grey bg-white shadow-warp-my'>
				<view class='flex flex-sub flex-direction solid-right'>
					<view class="text-xxl text-orange">25</view>
					<view class="margin-top-sm">
						<text class='cuIcon-hot'></text> Gitee</view>
				</view>
				<view class='flex flex-sub flex-direction solid-right'>
					<view class="text-xxl text-orange">99+</view>
					<view class="margin-top-sm">
        ...
        <view class="cu-item">
					<button class='content cu-btn' @click="goCode">
						<image src='../../static/me/icon/loingout.png' class='png' mode='aspectFit'></image>
						<text class='text-lg margin-sm'>本站源码</text>
					</button>
				</view>
			</view>
		</block>
...
*/
    ...
    onLoad(){
      // this.userInfo =this.$storage.getLocalStorageSync('userInfo')
    },
    mounted() {
      // let that = this;
      // //通过uni自带api获取手机系统信息
      ...
      goLoginOut(){
        let that = this
        this.$toast.confirm('提示','退出登录？',true,data=>{
          if (data.confirm) {
            uni.clearStorageSync();//清除本地缓存
            // that.$jump.reLaunchTo('/pages/login/index')
          }
        })
      }
		}
