<template>
  <view class="content">
    <view class="section slogan">
      <view> 🌍 {{new Date().getFullYear()}}年 博文记录 共 {{count}} 条</view>
      <u-line margin="10px 0px"></u-line>
      <view class="tags-box">
        <view  class="tags-item"
               v-for="(item,index) in monthData"
               :key="index"
               >
          {{item.mouth}}月 | {{item.count}}
        </view>
      </view>
    </view>

    <u-divider :dot="true" :dashed="true"></u-divider>
    <TimeLine>
		<view class="time-line-box" v-for="(item,index) in dataList" :key="item.id">
			<view class="time-line-item">
				<view class="section-item"  @click="goPostDetail(item)">
    ...
    <u-divider text="END" :dashed="true"></u-divider>
<!--    <u-gap height="15" bgColor="#f2f5f9"></u-gap>-->
    <view class="section-list" v-if="false">
      <view class="section-item" v-for="item in dataList" :key="item.id" @click="goPostDetail(item)">
        ...
        <view class="list-desc">
          ...
          <view class="desc-time">
            <u-icon name="clock" size="12"></u-icon>
            <text class="time">{{$tool.fmtLong2DateTime(item.updatedAt)}}</text>
          </view>
        </view>
      </view>
    </view>
...
<script>
import {dirArticle, getArticleGroup} from "../../api/mini.article";
